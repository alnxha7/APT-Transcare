...
def employee_create(request):
    ...
    if request.method == "POST":
        ...
        fixed_km = request.POST.get('fixed_km')
        ...
        Employee_master.objects.create(
            ...
            km_rate=km_rate if km_rate else 0.00, 
            fixed_km=fixed_km if fixed_km else 0.00, 
            fixed_charge=fixed_charge if fixed_charge else 0.00, 
            additional_charge=additional_charge if additional_charge else 0.00, 
...
def employee_update(request, pk):
    ...
    if request.method == "POST":
        ...
        employee.km_rate=km_rate
        employee.fixed_km=fixed_km
        employee.fixed_charge=fixed_charge
        employee.additional_charge=additional_charge
    ...
    try:
        if vehicle_id:
            ...
            if customer_id:
                ...
                monthly_child = RateChild.objects.filter(
                ...
                km_child = RateChild.objects.filter(
                    vehicle=vehicle.vehicle_type.vehicle_name,
                    type="Km Wise",
                    master__customer_name=customer.head,
                    master__company=company
                ...
                if monthly_child:
                    response_data["monthly_rate"] = monthly_child.rate

                if km_child:
                    response_data["rate"] = km_child.rate
...
def get_monthly_rate(request):
    customer_id = request.GET.get("customer_id")
    vehicle_id = request.GET.get("vehicle_id")
    co_id = request.session.get("co_id")

    try:
        rate_child = RateChild.objects.get(
            master__customer_name=customer_id,
            vehicle=vehicle_id,
            type="Monthly"
        )
        return JsonResponse({
            "success": True,
            "km": rate_child.km,
            "fixed_rate": rate_child.fixed_rate,
            "additional_charge": rate_child.additional_charge
        })
    except RateChild.DoesNotExist:
        return JsonResponse({"success": False, "message": "No monthly rate found"})
...
def get_next_entry_number(request):
    ...
    if series_id:
        try:
            ...
            next_entry_number = series_instance.serial_no
...
def trip_update(request, series, entry_number):
    ...
    if request.method == "POST":
        ...
        for i in range(len(customer_name_list)):
            ...
            trip.fuel_station = request.POST.get('fuel_station')
...
from django.contrib import messages
from .models import (
    Table_Accountsmaster, Table_Companydetailsmaster, Table_BillMaster,
    Table_BillItems, VoucherConfiguration, Branch_master, Table_Acntchild,
    Trip_sheet
)
...
def bill_details(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'go':
            ...
            trips = (
                Trip_sheet.objects.filter(
                    account_code=customer_id,
                    customer_name=account_master,
                    rate_type=rate_type,
                    loading_date__range=(from_date_obj, to_date_obj)
                )
                .values(
                    "vehicle_number_id", 
                    "vehicle_number_id__registration_number", 
                    "vehicle_type_id__vehicle_name"
                )
                .annotate(
                    total_km=Sum("total_km"),
                    km_charge_total=Sum("km_charge_total"),
                    extra_hour_charge_total=Sum("extra_hour_charge_total"),
                    fixed_charge_total=Sum("fixed_charge_total"),
                    haulting_charge_total=Sum("haulting_charge_total"),
                    toll_parking_total=Sum("toll_parking_total"),
                    freight_total=Sum("total_freight_charges"),
                )
            )
            ...
            try:
                ...
                try:
                    ...
                    total_kms = request.POST.getlist("total_km[]")
                    km_rates = request.POST.getlist("km_rate[]")
                    fixed_kms = request.POST.getlist("fixed_km[]")
                    additional_kms = request.POST.getlist("additional_km[]")
                    total_charges = request.POST.getlist("total_charges[]")
                    extra_charges = request.POST.getlist("extra_charges[]")
                    fixed_charges = request.POST.getlist("fixed_charges[]")
                    toll_parkings = request.POST.getlist("toll_parking[]")
                    totals = request.POST.getlist("total_freight_charges[]")

                    for i in range(len(vehicle_nos)):
                        Table_BillItems.objects.create(
                            master=master,
                            vehicle_no=vehicle_nos[i],
                            vehicle_type=vehicle_types[i],
                            total_km=total_kms[i] or 0,
                            km_rate=km_rates[i] or 0,
                            fixed_km=fixed_kms[i] or 0,
                            additional_km=additional_kms[i] or 0,
                            total_charges=total_charges[i] or 0,
                            additional_charge=extra_charges[i] or 0,
                            fixed_or_rental_charge=fixed_charges[i] or 0,
                            toll_parking=toll_parkings[i] or 0,
                            total=totals[i] or 0,
                        )
                    ...
                    context = {
                        ...
                        'master': master,
                        "bill_date": datetime.strptime(str(master.bill_date), "%Y-%m-%d").strftime("%d-%m-%Y"),
                        "to_date": datetime.strptime(str(master.date_to), "%Y-%m-%d").strftime("%d-%m-%Y"),
                        "from_date": datetime.strptime(str(master.date_from), "%Y-%m-%d").strftime("%d-%m-%Y"),
...
def bill_details_ajax(request):
    if request.method == "POST":
        customer_id = request.POST.get("customer_id")
        customer_name = Table_Accountsmaster.objects.filter(account_code=customer_id).first()
        from_date = request.POST.get("from_date")
        to_date = request.POST.get("to_date")
        ...
        trips = Trip_sheet.objects.filter(
            account_code=customer_id,
            loading_date__range=(from_date_obj, to_date_obj),
        )
        ...
        # ---------- First loop: accumulate raw values ----------
        for trip in trips:
            vno = trip.vehicle_number_id.registration_number
            vtype = trip.vehicle_type_id.vehicle_name

            if vno not in vehicles_data:
                vehicles_data[vno] = {
                    "vehicle_number": vno,
                    "vehicle_type": vtype,
                    "rate_type": trip.rate_type,
                    "total_km": 0,
                    "fixed_km": 0,
                    "additional_km": 0,
                    "fixed_charge_total": 0,
                    "extra_hour_charge_total": 0,
                    "toll_parking_total": 0,
                    "total_freight_charges": 0,   # ? initialize
                    "freight_total": 0,
                    "km_charge_total": trip.km_rate,
                }

            vehicles_data[vno]["total_km"] += trip.total_km or 0
            vehicles_data[vno]["toll_parking_total"] += trip.toll_parking_total or 0
            vehicles_data[vno]["total_freight_charges"] += (trip.total_km * trip.km_rate) + trip.fixed_charge_total

            # For Non-Monthly trips, sum charges
            if trip.rate_type != "Monthly":
                fixed_charge = trip.fixed_charge_total or 0
                extra_charge = trip.extra_hour_charge_total or 0
                tolls = trip.toll_parking_total or 0

                vehicles_data[vno]["fixed_charge_total"] += fixed_charge
                vehicles_data[vno]["extra_hour_charge_total"] += extra_charge
                vehicles_data[vno]["freight_total"] += trip.total_freight_charges

        # ---------- Second loop: handle Monthly vehicles ----------
        for vno, data in vehicles_data.items():
            if data["rate_type"] == "Monthly":
                try:
                    rate = RateChild.objects.get(
                        master__customer_name=customer_name.head,
                ...
                fixed_km = rate.km or 0
                fixed_rate = rate.fixed_rate or 0
                add_charge = rate.additional_charge or 0

                data["fixed_km"] = fixed_km
                data["fixed_charge_total"] = fixed_rate

                if total_km > fixed_km:
                    data["additional_km"] = total_km - fixed_km
                    data["extra_hour_charge_total"] = data["additional_km"] * add_charge
                else:
                    data["additional_km"] = 0
                    data["extra_hour_charge_total"] = 0

                # Rental = fixed + additional
                rental_total = fixed_rate + data["extra_hour_charge_total"]
                data["rental_total"] = rental_total

                # Freight = rental + tolls
                data["freight_total"] = rental_total + data["toll_parking_total"]

                # Optional: total_freight_charges same as freight_total
                data["total_freight_charges"] = rental_total

        return JsonResponse(list(vehicles_data.values()), safe=False)
        ...
        if action == 'save' or action == 'print':
            ...
            try:
                ...
                try:
                    ...
                    context = {
                        'master': bill_master,
                        'children': Table_BillItems.objects.filter(master=bill_master),
                        'trips': trips,
                        'print': True if action == 'print' else False,
                        'company': company,
                        'status': 'success'
...
class AccountDebitNoteView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        co_id = self.request.session.get('co_id')
        branch_name = self.request.session.get('branch')
        fy_year = self.request.session.get('fycode')

        company = Table_Companydetailsmaster.objects.filter(company_id=co_id).first()
        branch = Branch_master.objects.filter(branch_name=branch_name).first()

        
        logger.debug(f'Getting context data for series: {series}, serial_no: {serial_no}')

        context['series_options'] = VoucherConfiguration.objects.filter(
            category='Debit Note',
            fy_year=self.request.session.get('fycode'),
            company=company,
            branch=branch
        ).values('series', 'serial_no')        
    ...
    def create_new_entries(self, request, series, date, head1, narration1, debit1, head2, narration2, credit2):
        ...
        company = Table_Companydetailsmaster.objects.filter(company_id=co_id).first()
        branch = Branch_master.objects.filter(branch_name=branch_name).first()
        voucher_config = VoucherConfiguration.objects.select_for_update().filter(
            series=series,
            category='Debit Note',
            fy_year=self.request.session.get('fycode'),
            company=company,
        ...
        company = Table_Companydetailsmaster.objects.filter(company_id=(self.request.session.get('co_id'))).first()
        coid_entry = Table_companyDetailschild.objects.filter(company_id=company, fycode=(self.request.session.get('fycode'))).first()
        fycode_entry = coid_entry
        coid = coid_entry.company_id.company_id if coid_entry else 'C'
...
class AccountDebitTableView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        company = Table_Companydetailsmaster.objects.filter(company_id=(self.request.session.get('co_id'))).first()
        return Table_DrCrNote.objects.filter(ntype='D', user__company=company, fycode=self.request.session.get('fycode'))
...
class SearchDebitTableView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if series and serial_no:
            exists = Table_DrCrNote.objects.filter(
                series=series,
                noteno=serial_no,
                ntype='C',
                user=request.user,
...
class AccountCreditTableView(ListView):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(ntype='C', user=self.request.user, fycode=self.request.session.get('fycode'))
...
class SearchCreditTableView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if series and serial_no:
            ...
            if exists:
                return redirect(
                    reverse('main:account_credit_note', kwargs={'series': series, 'serial_no': serial_no})
                )
            else:
                context = self.get_context_data(**kwargs)
                context['error'] = 'This serial number does not exist.'
...
class DeleteCreditNoteView(LoginRequiredMixin, DeleteView):
    ...
    def get_object(self, queryset=None):
        ...
        # Fetch the object
        obj = Table_DrCrNote.objects.filter(id=pk1, noteno=pk2, user=self.request.user, ntype='C', fycode=self.request.session.get('fycode')).first()
        if not obj:
            raise Http404("No Table_DrCrNote matches the given query.")
        return obj
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Optionally, handle the second object if needed
        obj2 = Table_DrCrNote.objects.filter(noteno=self.object.noteno, user=self.request.user, ntype='C', fycode=self.request.session.get('fycode')).exclude(id=self.object.id).first()
        if obj2:
            dramount2 = float(obj2.dramount) if obj2.dramount else 0
...
from .models import Table_Voucher
...
class EnterAmountView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        vouchers = VoucherConfiguration.objects.filter(
            category='receipt',
            company=company,
            branch=branch,
            fy_year=self.request.session.get('fycode')
        ...
        table_acntchildren = Table_Acntchild.objects.select_related('account_master').filter(
            account_master__category__in=['Bank', 'Cashbook'], account_master__user=request.user, fyc_code=self.request.session.get('fycode')
        )
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            user_id = request.user.id
            fy_code = request.session.get('fycode')
            coid = request.session.get('co_id')
            branch_id = request.session.get('branch')
            ...
            voucher_config = VoucherConfiguration.objects.get(series=series, category='receipt', fy_year=fy_code)
            ...
            with transaction.atomic():
                for accountcode, narration, vtype, payment in zip(accountcodes, narrations, vtypes, payments):
                    ...
                    try:
                        ...
                        acnt_child = Table_Acntchild.objects.get(account_code=accountcode, account_master=account_master, fyc_code=(request.session.get('fycode')))
                        if acnt_child:
            ...
            return render(request, self.template_name, {
                ...
                'vouchers': VoucherConfiguration.objects.filter(category='receipt', fy_year=self.request.session.get('fycode')),
                ...
                'table_acntchildren': Table_Acntchild.objects.select_related('account_master').filter(
                    account_master__category__in=['Bank', 'Cashbook'], account_master__user=self.request.user, fyc_code=self.request.session.get('fycode')
                ),
                'user_id': request.user.id,
            })
        except Exception as e:
...
class EditReceiptView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        voucher_entries = Table_Voucher.objects.filter(Series=series, VoucherNo=voucher_no, CStatus='R', user=self.request.user, FYCode=self.request.session.get('fycode'))
            ...
            with transaction.atomic():
                ...
                for accountcode, narration, vtype, payment in zip(accountcodes, narrations, vtypes, payments):
                    Table_Voucher.objects.create(
                        ...
                        FYCode=request.session.get('fycode'),
                        Coid=request.session.get('coid'),
                        Branch_ID=request.session.get('branch')
                    )

                    # Update the current balance for Table_Acntchild
                    try:
                        acnt_child = Table_Acntchild.objects.get(account_code=accountcode, account_master__user=self.request.user, FYCode=self.request.session.get('fycode'))
                        if acnt_child:
                            current_balance = float(acnt_child.current_balance or 0)
...
class ReceiptDetailView(View):
    ...
    def get_queryset(self):
        receipt_series = VoucherConfiguration.objects.filter(category='receipt', fy_year=self.request.session.get('fycode')).values_list('series', flat=True)

        # Filter Table_Voucher based on the series related to the receipt category and CStatus being 'R'
        return Table_Voucher.objects.filter(Series__in=receipt_series, CStatus='R', user=self.request.user, FYCode=self.request.session.get('fycode'))
...
class PaymentEnterAmountView(LoginRequiredMixin, View):
    ...
    def update_current_balance(self, account_code, payment, increase=True):
        try:
            acnt_child = Table_Acntchild.objects.filter(account_code=account_code,  account_master__user=self.request.user, fyc_code=self.request.session.get('fycode')).first()
            current_balance = float(acnt_child.current_balance or 0)
            payment_amount = float(payment)
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            user_id = request.user.id
            fy_code = self.request.session.get('fycode')
            coid = self.request.session.get('co_id')
            ...
            voucher_config = VoucherConfiguration.objects.get(series=series, category='payment', fy_year=fy_code)
            current_serial_no = int(voucher_config.serial_no)
            voucher_no = current_serial_no
            ...
            return render(request, self.template_name, {
                ...
                'vouchers': VoucherConfiguration.objects.filter(category='payment', fy_year=self.request.session.get('fycode')),
                'accountmas': Table_Accountsmaster.objects.filter(category__in=['Cashbook', 'Bank'], user=self.request.user),
                'head_accounts': Table_Accountsmaster.objects.exclude(category__in=['Cashbook', 'Bank']).filter(user=self.request.user),
                'next_serial_numbers': next_serial_numbers,
                'table_acntchildren': Table_Acntchild.objects.select_related('account_master').filter(
            ...
            next_serial_numbers = {
                voucher.series: str(voucher_config.serial_no)
                for voucher in VoucherConfiguration.objects.filter(category='payment', user=self.request.user, fy_year=self.request.session.get('fycode'))
            }
            return render(request, self.template_name, {
                'error': str(e),
                'vouchers': VoucherConfiguration.objects.filter(category='payment', fy_year=self.request.session.get('fycode')),
...
class EditPaymentView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Fetch voucher entries for the given series and voucher number
        voucher_entries = Table_Voucher.objects.filter(Series=series, VoucherNo=voucher_no, CStatus='P', user=self.request.user, FYCode=self.request.session.get('fycode'))
        ...
        if not voucher_entries:
            ...
            return render(request, 'accounts/payment/payment_modify.html', {
                ...
                'user_id': request.user.id,
                'error_message': 'This combination of Series and Voucher No does not exist.',
                'redirect_url': reverse('main:payment_modify')  # Replace with the correct URL name
            })
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            user_id = request.user.id
            fy_code = self.request.session.get('fycode')
            coid = self.request.session.get('coid')
...
#                 return redirect(reverse('main:account_debit_note', kwargs={'series': series, 'voucher_no': voucher_no}))
#             else:
#                 context = self.get_context_data(**kwargs)
...
class PaymentListTable(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        payment_series = VoucherConfiguration.objects.filter(category='payment', fy_year=self.request.session.get('fycode')).values_list('series', flat=True)
        
        # Filter Table_Voucher based on the series related to the payment category and CStatus being 'R'
        return Table_Voucher.objects.filter(Series__in=payment_series, CStatus='P', user=self.request.user, FYCode=self.request.session.get('fycode'))
...
class JournalEntryView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for voucher in vouchers:
            last_serial = Table_Journal_Entry.objects.filter(series=voucher.series, auth_user=self.request.user, fycode=self.request.session.get('fycode')).order_by('-voucher_no').first()
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            vdate = request.POST.get('vdate')
            user_id = request.user.id
            branch = Branch_master.objects.first()  
            branch_id = request.session.get('branch')
            ...
            with transaction.atomic():
                ...
                voucher_config = VoucherConfiguration.objects.get(series=series, category='Journal Entry', fy_year=request.session.get('fycode'))
...
class JournalEntryTable(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(auth_user=self.request.user, fycode=self.request.session.get('fycode'))
...
class EditJournalEntry(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        vouchers = VoucherConfiguration.objects.filter(category='Journal Entry', fy_year=request.session.get('fycode'))
        accountmas = Table_Accountsmaster.objects.filter(category__in=['Bank', 'Cashbook'], user=self.request.user)
        wallet = Wallet.objects.first()
        head_accounts = Table_Accountsmaster.objects.exclude(category__in=['Bank', 'Cashbook']).filter(user=self.request.user)
        table_acntchildren = Table_Acntchild.objects.select_related('account_master').filter(
            account_master__category__in=['Bank', 'Cashbook'], account_master__user=self.request.user, fyc_code=request.session.get('fycode')
        )

        voucher_entries = Table_Journal_Entry.objects.filter(series=series, voucher_no=voucher_no, auth_user=self.request.user, fycode=request.session.get('fycode'))
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            with transaction.atomic():
                # Fetch existing entries and compute old debit and credit totals
                existing_entries = Table_Journal_Entry.objects.filter(series=series, voucher_no=voucher_no, auth_user=self.request.user, fycode=request.session.get('fycode'))
                old_account_updates = {}
                for entry in existing_entries:
                    if entry.accountcode not in old_account_updates:
                        old_account_updates[entry.accountcode] = Decimal('0')
...
class DeleteJournalEntryView(LoginRequiredMixin, View):
    def delete(self, request, voucher_no, *args, **kwargs):
        try:
            ...
            entries = Table_Journal_Entry.objects.filter(voucher_no=voucher_no, auth_user=self.request.user, fycode=request.session.get('fycode'))
...
class ContraEntryView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        next_serial_numbers = {}
        for voucher in vouchers:
            last_serial = Table_Contra_Entry.objects.filter(series=voucher.series, auth_user=self.request.user, fycode=request.session.get('fycode')).order_by('-voucher_no').first()
            next_serial_numbers[voucher.series] = str(int(last_serial.voucher_no) + 1) if last_serial else voucher.serial_no
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            with transaction.atomic():
                ...
                voucher_configs = VoucherConfiguration.objects.filter(series=series, category='Contra Entry', fy_year=request.session.get('fycode'))
...
class EditContraEntry(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        vouchers = VoucherConfiguration.objects.filter(category='Contra Entry', fy_year=request.session.get('fycode'))
        accountmas = Table_Accountsmaster.objects.filter(category__in=['Accounts', 'Customers', 'Suppliers'], user=self.request.user)
        head_accounts = Table_Accountsmaster.objects.exclude(category__in=['Accounts', 'Customers', 'Suppliers']).filter(user=self.request.user)
        table_acntchildren = Table_Acntchild.objects.filter(account_master__category__in=['Accounts', 'Customers', 'Suppliers'], 
                                                            account_master__user=self.request.user, fyc_code=request.session.get('fycode'))

        voucher_entries = Table_Contra_Entry.objects.filter(series=series, voucher_no=voucher_no,  auth_user=self.request.user, fycode=request.session.get('fycode'))
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            voucher_config = VoucherConfiguration.objects.get(series=series, category='Contra Entry', fy_year=request.session.get('fycode'))

            with transaction.atomic():
                # Fetch existing entries and compute old debit and credit totals
                existing_entries = Table_Contra_Entry.objects.filter(series=series, voucher_no=voucher_no, auth_user=self.request.user, fycode=request.session.get('fycode'))
                old_account_updates = {}
                for entry in existing_entries:
                    if entry.accountcode not in old_account_updates:
                        old_account_updates[entry.accountcode] = Decimal('0')
    ...
    def delete(self, request, voucher_no, *args, **kwargs):
        try:
            with transaction.atomic():
                ...
                contra_entries = Table_Contra_Entry.objects.filter(voucher_no=voucher_no, auth_user=self.request.user, fycode=request.session.get('fycode'))
...
class LedgerView(LoginRequiredMixin, View):
    def get(self, request, account_code, start_date, end_date):
        ...
        previous_entries = (
            list(Table_Voucher.objects.filter(Accountcode=account_code, Vdate__lt=start_date, FYCode=request.session.get('fycode'))) +
            list(Table_DrCrNote.objects.filter(accountcode=account_code, ndate__lt=start_date, fycode=request.session.get('fycode'))) +
            list(Table_Journal_Entry.objects.filter(accountcode=account_code, vdate__lt=start_date, fycode=request.session.get('fycode'))) +
            list(Table_Contra_Entry.objects.filter(accountcode=account_code, vdate__lt=start_date, fycode=request.session.get('fycode')))
        )
        ...
        voucher_entries = Table_Voucher.objects.filter(
            Accountcode=account_code, Vdate__range=[start_date, end_date], user=self.request.user, FYCode=request.session.get('fycode')
        )

        drcr_entries = Table_DrCrNote.objects.filter(
            accountcode=account_code, ndate__range=[start_date, end_date], user=self.request.user, fycode=request.session.get('fycode')
        )

        journal_entries = Table_Journal_Entry.objects.filter(
            accountcode=account_code, vdate__range=[start_date, end_date], auth_user=self.request.user, fycode=request.session.get('fycode')
        )

        contra_entries = Table_Contra_Entry.objects.filter(
            accountcode=account_code, vdate__range=[start_date, end_date], auth_user=self.request.user, fycode=request.session.get('fycode')
        )
        bill_entries = Table_BillMaster.objects.filter(
            customer__account_code=account_code,
        ...
        for entry in combined_entries:
            ...
            if hasattr(entry, "Vdate"):
                ...
                debit_amount = Decimal(entry.grand_total) if entry.grand_total >= 0 else Decimal("0.00")
                credit_amount = abs(Decimal(entry.grand_total)) if entry.grand_total < 0 else Decimal("0.00")
            
                entry_dict = {
                    "date": entry.bill_date,
                    "voucher_number": f"Bill-{entry.bill_no}",
                    "narration": "Bill Issued",
                    "debit": debit_amount,
                    "credit": credit_amount,
                    "is_bill_entry": True,
                }      
...
class CashBookView(LoginRequiredMixin, View):
    def get(self, request, account_code, start_date, end_date):
        ...
        def get_last_closing_balance():
            entries = (
                list(Table_Voucher.objects.filter(Accountcode=account_code, Vdate__lt=start_date, user=self.request.user, FYCode=request.session.get('fycode'))) +
                list(Table_DrCrNote.objects.filter(accountcode=account_code, ndate__lt=start_date, user=self.request.user, fycode=request.session.get('fycode'))) +
                list(Table_Journal_Entry.objects.filter(accountcode=account_code, vdate__lt=start_date, auth_user=self.request.user, fycode=request.session.get('fycode'))) +
                list(Table_Contra_Entry.objects.filter(accountcode=account_code, vdate__lt=start_date, auth_user=self.request.user, fycode=request.session.get('fycode')))
            )
        ...
        voucher_entries = Table_Voucher.objects.filter(
            ...
            user=self.request.user,  FYCode=request.session.get('fycode')
        ).order_by("Vdate")
...
class BankBookView(LoginRequiredMixin, View):
    def get(self, request, account_code, start_date, end_date):
        ...
        voucher_entries = Table_Voucher.objects.filter(
            Q(Accountcode=account_code) | Q(Headcode=account_code),
            Vdate__range=[start_date, end_date], user=self.request.user, FYCode=request.session.get('fycode')
        ).order_by("Vdate")
...
class DayBookView(LoginRequiredMixin, View):
    def get(self, request, start_date, end_date):
        ...
        voucher_entries = Table_Voucher.objects.filter(Vdate__range=[start_date, end_date], user=self.request.user, FYCode=request.session.get('fycode')).order_by("Vdate")
        drcr_entries = Table_DrCrNote.objects.filter(ndate__range=[start_date, end_date], user=self.request.user, fycode=request.session.get('fycode')).order_by("ndate")
        journal_entries = Table_Journal_Entry.objects.filter(vdate__range=[start_date, end_date], auth_user=self.request.user, fycode=request.session.get('fycode')).order_by("vdate")
        contra_entries = Table_Contra_Entry.objects.filter(vdate__range=[start_date, end_date], auth_user=self.request.user, fycode=request.session.get('fycode')).order_by("vdate")
...
class TrialBalanceView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        voucher_entries = Table_Voucher.objects.filter(user=self.request.user, FYCode=self.request.session.get('fycode')).order_by("Vdate")
        drcr_entries = Table_DrCrNote.objects.filter(user=self.request.user, fycode=self.request.session.get('fycode')).order_by("ndate")
        journal_entries = Table_Journal_Entry.objects.filter(auth_user=self.request.user, fycode=self.request.session.get('fycode')).order_by("vdate")
        contra_entries = Table_Contra_Entry.objects.filter(auth_user=self.request.user, fycode=self.request.session.get('fycode')).order_by("vdate")
...
class AmountReceivableSearchOutstandingSearchView(LoginRequiredMixin, View):
    def get(self, request):
        # Fixed group
        selected_group = "SUNDRY DEBTORS"
        selected_head = request.GET.get("head", "")

        # Fetch heads only for SUNDRY DEBTORS
        heads = Table_Accountsmaster.objects.filter(
            user=request.user,
        ...
        return render(request, "accounts/outstanding_analysis/amount_receivable_search.html", context)


class AmountPayableSearchOutstandingSearchView(LoginRequiredMixin, View):
    def get(self, request):
        # Fixed group
        selected_group = "SUNDRY CREDITORS"
        selected_head = request.GET.get("head", "")

        # Fetch heads only for SUNDRY CREDITORS
        heads = Table_Accountsmaster.objects.filter(
            user=request.user,
            group=selected_group
        ).order_by("head")
        ...
        return render(request, "accounts/outstanding_analysis/amount_payable_search.html", context)   



class OutstandingAnalysis(LoginRequiredMixin, View):
    def get(self, request):
        company_details = Table_Companydetailsmaster.objects.first()

        selected_group = request.GET.get("group", "Sundry Debtors")
        selected_head = request.GET.get("head", "")

        accounts = Table_Accountsmaster.objects.filter(user=request.user).order_by("group", "head")
        if selected_group:
            accounts = accounts.filter(group=selected_group)
        if selected_head:
            accounts = accounts.filter(head=selected_head)

        voucher_entries = Table_Voucher.objects.filter(
            user=request.user, FYCode=request.session.get("fycode")
        ).order_by("Vdate")
        drcr_entries = Table_DrCrNote.objects.filter(
            user=request.user, fycode=request.session.get("fycode")
        ).order_by("ndate")
        journal_entries = Table_Journal_Entry.objects.filter(
            auth_user=request.user, fycode=request.session.get("fycode")
        ).order_by("vdate")
        contra_entries = Table_Contra_Entry.objects.filter(
            auth_user=request.user, fycode=request.session.get("fycode")
        ).order_by("vdate")

        combined_entries = list(voucher_entries) + list(drcr_entries) + list(journal_entries) + list(contra_entries)
        combined_entries.sort(
            key=lambda x: getattr(x, "Vdate", getattr(x, "ndate", getattr(x, "vdate", datetime.min.date())))
        )

        entry_list = []
        group_totals = {}
        total_current_balance = Decimal("0.00")  # Track total current balance

        for account in accounts:
            current_balance = Decimal(account.currentbalance or "0.00")
            total_current_balance += current_balance  # accumulate total
            ...
            for entry in combined_entries:
                if hasattr(entry, "Accountcode") and entry.Accountcode == account.account_code:
                    ...
                    account_debit += Decimal(entry.dramount or "0.00")
                    account_credit += Decimal(entry.cramount or "0.00")
        ...
        for group, totals in group_totals.items():
            for entry in entry_list:
                if entry["group"] == group:
                    entry["debit"] += totals['debit']
                    entry["credit"] += totals['credit']
                    break

        context = {
            "entries": entry_list,
            "total_current_balance": total_current_balance,  # pass total current balance to template
            "company_name": company_details.companyname,
            "company_phone": company_details.phoneno,
            "company_address": company_details.address1,
            "company_gst": company_details.gst,
        }
        return render(request, "accounts/outstanding_analysis/outstandinganalysis.html", context)
...
def item_add(request):
    ...
    company = Table_Companydetailsmaster.objects.get(company_id=co_id)
    gsts = TaxMaster.objects.filter(
        company=company,
        fy_code=fycode,
        tax_category__in=["TAXABLE", "NON TAXABLE"]
    )
    units=UnitMaster.objects.filter(company=company,fy_code=fycode)
    groups=GroupMaster.objects.filter(company=company,fy_code=fycode)
    
    if request.method == 'POST':

        
        if Item_master.objects.filter(name=request.POST.get('name'),branch_id=branch_name,co_id=co_id,financial_year=fycode).exists():
             return render(request, 'item/item_form.html', {
                'name_error': 'Item Name  already exists.','gsts':gsts,'units':units,'groups':groups,
                
            })

         # Generate the next brand_custom_id for this co_id + branch_id
        max_id = Item_master.objects.filter(co_id=co_id, branch_id=branch).aggregate(Max('item_code'))[
                     'item_code__max'] or 0
        print("max id",max_id)
        next_custom_id = max_id + 1

        unit_id=request.POST.get('unit')
        unit = get_object_or_404(UnitMaster, pk=unit_id)

       
        group_id = request.POST.get('group')
        group = None
        if group_id:
            group = get_object_or_404(GroupMaster, pk=group_id)	
	

        item = Item_master(
            item_code=next_custom_id,
            name=request.POST.get('name').upper(),
            unit=unit,
            group=group,
            sub_group=request.POST.get('sub_group'),
            reorder=request.POST.get('reorder') or 0,
            location=request.POST.get('location'),
            min_level=request.POST.get('min_level') or 0,
            godown=request.POST.get('godown'),
            hsn=request.POST.get('hsn'),
            max_level=request.POST.get('max_level') or 0,
        ...
        # Now create Stock using POST values
        gst_id=request.POST.get('gst')
        gst = get_object_or_404(TaxMaster, pk=gst_id)

        Stock.objects.create(
            item_code=next_custom_id,
            item=item,
            batch_no=request.POST.get('batch_no') or 'DEFAULT',
            exp_date=request.POST.get('exp_date') or '2099-12-31',
            p_price=request.POST.get('p_price') or 0,
            price_1=request.POST.get('price_1') or 0,
            price_2=request.POST.get('price_2') or 0,
            price_3=request.POST.get('price_3') or 0,
            mrp=request.POST.get('mrp') or 0,
            gst=gst,
            tax_code=gst.tax_code,
            op_stock=request.POST.get('op_stock') or 0,
            op_lc=request.POST.get('op_lc') or 0,
            stock=request.POST.get('stock') or 0,
            land_coast=request.POST.get('land_coast') or 0,
            co_id=co_id,
            branch_id=branch,
            financial_year=fycode
        )

        return render(request, 'item/item_form.html', {"success": True,'gsts':gsts,'units':units,'groups':groups,})
  
    return render(request, 'item/item_form.html',{'gsts':gsts,'units':units,'groups':groups,})
...
def item_list(request):
    ...
    query = request.GET.get('q')
    if query:
        stocks = Stock.objects.filter(item__name__iexact=query,co_id=co_id,branch_id=branch,financial_year=fycode)
    else:
        
        stocks = Stock.objects.filter(co_id=co_id,branch_id=branch,financial_year=fycode)
        Item_mastere = Item_master.objects.filter(co_id=co_id,branch_id=branch,financial_year=fycode)
    return render(request, 'item/item_list.html', {'stocks': stocks, 'Item_mastere': Item_mastere, 'query': query})

def item_readonly(request,pk):
    co_id = request.session.get('co_id')
    branch = request.session.get('branch')

    item = get_object_or_404(Item_master, pk=pk)
    stock = get_object_or_404(Stock,pk=pk)

    units = UnitMaster.objects.all()
    groups = GroupMaster.objects.all()
    gsts= TaxMaster.objects.all()

    return render(request,'item/item_edit.html',{'item':item,"stock":stock,  'units': units,'gsts':gsts,'groups':groups,})


def item_delete(request, pk):
    item = get_object_or_404(Item_master, pk=pk)
    item.delete()  # Automatically deletes related Stock(s) due to CASCADE
...
def item_update(request, pk):
    ...
    if request.method == 'POST':
        if Item_master.objects.filter(name=request.POST.get('name')).exclude(pk=pk).exists():
            return render(request, 'item/item_form.html', {
                'name_error': 'Item Name  already exists.','gsts':gsts,'units':units,'groups':groups,
        ...
        item.item_code = request.POST.get('item_code') or None
        item.name = request.POST.get('name')

        unit_id = request.POST.get('unit')
        item.unit = get_object_or_404(UnitMaster, pk=unit_id) if unit_id else None

        group_id = request.POST.get('group')
        item.group = get_object_or_404(GroupMaster, pk=group_id) if group_id else None

        # item.unit = request.POST.get('unit')
        # item.group = request.POST.get('group')
        item.sub_group = request.POST.get('sub_group')
        item.reorder = request.POST.get('reorder') or 0
        item.location = request.POST.get('location')
        item.min_level = request.POST.get('min_level') or 0
        item.godown = request.POST.get('godown')
        item.hsn = request.POST.get('hsn')
        item.max_level = request.POST.get('max_level') or 0
        item.description = request.POST.get('description')
        item.save()

        if stock:
            stock.batch_no = request.POST.get('batch_no') or "DEFAULT"
            stock.exp_date = request.POST.get('exp_date') or "2099-12-31"
            stock.p_price = request.POST.get('p_price') or 0
            stock.price_1 = request.POST.get('price_1') or 0
            stock.price_2 = request.POST.get('price_2') or 0
            stock.price_3 = request.POST.get('price_3') or 0
            stock.mrp = request.POST.get('mrp') or 0

            gst_id = request.POST.get('gst')
            stock.gst = get_object_or_404(TaxMaster, pk=gst_id) if gst_id else None
            # stock.gst = request.POST.get('gst') or 0
            stock.op_stock = request.POST.get('op_stock') or 0
            stock.op_lc = request.POST.get('op_lc') or 0
            stock.land_coast = request.POST.get('land_coast') or 0
            stock.save()
        # else:
        #     Stock.objects.create(
        #         item=item,
        ...
        #     )
        update='update'
        return render(request,'item/item_form.html',{'updation':True,'update': update, 'units': units,'gsts':gsts,'groups':groups,})

    return render(request, 'item/item_form.html', {'updation':True,'item': item, 'stock': stock, 'units': units,'gsts':gsts,'groups':groups,})
...
def location_list(request):
    locations = LocationMaster.objects.filter(company__company_id=request.session.get('co_id'), branch=Branch_master.objects.get(branch_name=request.session.get('branch')),)
...
def location_edit(request, location_id):
    ...
    return render(request, 'accounts/location_master/location_edit.html', {'location': location})
...
def driver_bata(request):
    drivers = Employee_master.objects.filter(
        id__in=Trip_sheet.objects.values_list("driver_name_id", flat=True).distinct()
    )
    if request.method == "POST":
        ...
        if driver_id and date_from and date_to:
            employee = Employee_master.objects.filter(employee_name=driver_id).first()
            trips = Trip_sheet.objects.filter(
                driver_name_id__employee_name=driver_id,
                loading_date__range=[date_from, date_to]
            )
            return render(request, "driver_bata/driver_bata.html", {"driver": driver_id, "trips": trips, "fixed_charge": employee.fixed_charge, "employee": employee,
...
def lorry_receipt(request):
    ...
    if request.method == 'POST':
        try:
            action = request.POST.get('action')
            series_id = request.POST.get('series')
            voucher_config = VoucherConfiguration.objects.get(id=series_id, category='Lorry Receipt')
            serial_no = voucher_config.serial_no

            lorry_receipt = LorryReceiptMaster.objects.create(
                company=Table_Companydetailsmaster.objects.get(company_id=request.session.get('co_id')),
                branch=Branch_master.objects.get(branch_name=request.session.get('branch')),
                fy_code=request.session.get('fycode'),
                ...
                lr_no=serial_no,
                lr_date=request.POST.get('bill_date'),

                consigner_name=request.POST.get('consigner_name'),
                consigner_code=request.POST.get('consigner_code'),
                consigner_account=Table_Accountsmaster.objects.get(company__company_id=request.session.get('co_id'),
                                                    branch__branch_name=request.session.get('branch'), 
                                                    account_code=request.POST.get('consigner_code')),

                consignee_name=request.POST.get('consignee_name'),
                consignee_code=request.POST.get('consignee_code'),
            ...
            context = {
                ...
                'lr': lorry_receipt,
                'lr_items': LorryReceiptItems.objects.filter(master=lorry_receipt),
            }
...
def lr_search(request):
    ...
    return render(request, 'lorry_receipt/lr_search.html', {'series': series})
